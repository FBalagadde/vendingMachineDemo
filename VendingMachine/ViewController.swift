...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    @IBAction func purchase()
    {
        if let currentSelection = currentSelection
        {
            ...
            {
                showWlert(title: "Out of Stock", message: "This item is unavailable. Please make another selection")
                
            } catch VendingMachineError.invalidSelection
            {
                showWlert(title: "Invalid Selection", message: "Please make another selection")
                
            } catch VendingMachineError.insufficientFunds(let required)
            {
                let message = "You need $\(required) to complete this transaction"
                showWlert(title: "Insufficient Funds", message: message)
            } catch let error
            {
                fatalError("\(error)")
            }
    ...
    func showWlert(title: String, message: String, style: UIAlertControllerStyle = .alert)
    {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let currentSelection = currentSelection, let item = vendingMachine.item(forSelection: currentSelection)
        {
            ...
            updateDisplayWith(totalPrice: item.price * quantityStepper.value, itemPrice: item.price)
        }
